...
// 서블릿에서는 메서드 단위로 매핑이 안되고 클래스 단위로만 매핑이 된다. 
// 서블릿은 스프링에 비해서 클래스를 많이 만들어야하는 단점이 있다. 
// 서블릿은 HttpServlet을 상속받아야 하고 메서드는 꼭 service라는 이름으로 해야하고 매개변수도 request, response도 고정으로 적어야한다.
...
public class MyDispatcherServlet extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws IOException {
		Map    map = request.getParameterMap(); // year=2021&month=10&day=1가 맵형태로 가져오게 된다.
		Model  model = null;
		String viewName = "";
		...
		try {
			...
			for(int i=0;i<paramArr.length;i++) {
				...
				if(paramType==Model.class) {
					...
					String strValue = ((String[])value)[0];	// getParameterMap()에서 꺼낸 value는 String배열(= String[])이므로 변환 필요 
